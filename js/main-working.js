    ...
    const toggle = () => {
        ...
        jsToggles.forEach((jsToggle) => {
            ...
            const jsToggleListener = (event) => {
                ...
                siblings === 'true' ? container = currentTarget.parentNode.querySelector(toggleSelectorText) : container = document.querySelector(`${toggleSelectorText}`);
                    ...
                    window.addEventListener('mouseup', function(event){
                        ...
                        const isParent = (parents) => {
                            for(let i = 0; i < parents.length; i++) {
                                if (parents[i] === currentTarget) {
                                    return true;
                                } else continue;
                            }
                        }

                        if (!isClickInside && !isParent(event.path)) {
